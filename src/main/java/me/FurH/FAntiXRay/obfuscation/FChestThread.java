...
public class FChestThread implements Runnable {
    ...
    public void execute() {
        ...
        if (lastLoc.distanceSquared(player.getLocation()) >= config.proximity_distance) {
            update = true;
        }
    ...
    public void update() {
        FConfiguration config = FAntiXRay.getConfiguration();
        int radius = config.proximity_radius;
        ...
        for (int x = -radius; x <= radius; x++) {
            for (int y = -radius; y <= radius; y++) {
                for (int z = -radius; z <= radius; z++) {
                    ...
                    if (lastLoc.distanceSquared(newLoc) >= config.proximity_radius) {
                        continue;
                    }
                    ...
                    if (id == 54 || config.hidden_world.contains(id) || config.hidden_nether.contains(id)) {
