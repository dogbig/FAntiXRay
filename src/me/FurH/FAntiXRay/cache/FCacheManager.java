    ...
    public static void getCacheSizeTask() {
        Bukkit.getScheduler().runTaskTimerAsynchronously(FAntiXRay.getPlugin(), new Runnable() {
            ...
            public void run() {
                ...
                if (size > limit) {
                    FAntiXRay.getCommunicator().log("[TAG] The cache is too big, {0} of {1} allowed in {2} files, cleaning up!", FUtil.format(size), FUtil.format((limit * 1024 * 1024)), files.size());
                    clearCache();
                }
            }
        }, 3600 * 20, 3600 * 20);
